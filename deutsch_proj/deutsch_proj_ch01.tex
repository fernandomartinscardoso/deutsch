...
\subsubsection*{ERN}
\addcontentsline{toc}{subsubsection}{ERN}
\label{sssec:ern}

The ending ``ern'' usually sounds like ``an'', which is better transcribed in IPA as \textipa{[5n]}.

\begin{itemize}
    \item \textbf{gestern}: \textipa{["gEst5n]}
    \item \textbf{jammern}: \textipa{["jam5n]}
    \item \textbf{Ostern}: \textipa{["o:st5n]}
    \item \textbf{sondern}: \textipa{["zOnd5n]}
    \item \textbf{verlängern}: \textipa{[fE5"lEN5n]}
\end{itemize}

\subsubsection*{IG and NG}
\addcontentsline{toc}{subsubsection}{IG and NG}
\label{sssec:ig_ng}

The official Duden dictionary website \cite{duden2025} makes the following observation regarding the pronunciation of final ``ng'' and ``ig'':

\begin{quotation}
    Final ``-ng'' is often mistakenly pronounced like a ``k'' \textipa{[k]}. However, neither the word ``Zeitung'' (newspaper) nor ``Ring'' (ring) should have a ``k'' \textipa{[k]} sound at the end in standard pronunciation. The same applies to the letter sequence ``-ig'': Here, too, the final ``-g'' is not pronounced like a ``k'' \textipa{[k]} by professional German speakers, especially in northern Germany and beyond, particularly in numerals, but rather like the fricative in the word ``ich'' \textipa{[I\c{c}]}. This applies to words like ``König'' \textipa{["k{\o}ni\c{c}]}, ``Honig'' \textipa{["honi\c{c}]}, ``zwanzig'' \textipa{["tsvantsi\c{c}]}, ``eilig'' \textipa{["aIli\c{c}]}, ``sperrig'' \textipa{["SpEri\c{c}]}, and many other adjectives ending in ``-ig''. In southern and central Germany, as well as in Switzerland and eastern Austria, however, the pronunciation with ``k'' \textipa{[k]} or ``g'' \textipa{[g]} is also common. As soon as further letters are added through declension, the ``g'' is generally pronounced like \textipa{[g]}: ``die Könige'' \textipa{["k{\o}nig@]}, ``eilige Nachrichten'' \textipa{["aIlig@]}, ``in einer sperrigen Kiste'' \textipa{["spErig\s{n}]}. If the ending ``-ig'' is followed by the derivational suffix ``-lich'', the ``g'' is pronounced like a ``k'' \textipa{[k]}\footnote{Partial translation of the article \textit{Zweifelsfälle bei der Aussprache}, available in \url{https://www.duden.de/sprachwissen/sprachratgeber/Zweifelsfalle-bei-der-Aussprache}.}.
\end{quotation}

\subsubsection*{PF}
\addcontentsline{toc}{subsubsection}{PF}
\label{sssec:pf}

The German ``PF'' pronunciation is an affricate, a combination of a soft ``p'' sound and an ``f'' sound that are quickly contracted, almost like a single sound. It is formed by briefly closing the lips for the ``p'' and almost immediately transitioning to the ``f'' sound without making the ``p'' clearly audible.

\begin{itemize}
    \item \textbf{Kopf}: \textipa{[kOpf]}
    \item \textbf{Pfaffe}: \textipa{["pfaf@]}
    \item \textbf{Pfanne}: \textipa{["pfan@]}
    \item \textbf{Pferd}: \textipa{[pfe:5t]}
    \item \textbf{pflegen}: \textipa{[pfle:g\s{n}]}
\end{itemize}

\subsubsection*{TION}
\addcontentsline{toc}{subsubsection}{TION}
\label{sssec:tion}

The ending ``tion'' usually sounds like ``tsion''\footnote{The letter ``t'' is pronounced like the German ``z'' in this case.}, which is better transcribed in IPA as \textipa{[tsio:n]}.

\begin{itemize}
    \item \textbf{Aktion}: \textipa{[ak"tsio:n]}
    \item \textbf{Funktion}: \textipa{[funk"tsio:n]}
    \item \textbf{Organisation}: \textipa{[O5ganiza"tsio:n]}
    \item \textbf{Produktion}: \textipa{[prodUk"tsio:n]}
    \item \textbf{Station}: \textipa{[Sta"tsio:n]}
\end{itemize}

\section*{Ausgabe}
\addcontentsline{toc}{section}{Ausgabe}
\label{sec:ausgabe_aussprachekurs}

\begin{enumerate}
    \item Review all content, checking the pronunciation of all words in the examples using the website Forvo.
    \item Compare the pronunciation of short and long vowels in the words below:
    \begin{itemize}
        \item {bienen} and {binnen}
        \item {bieten} and {bitten}
        \item {den} and {denn}
        \item {ihm} and {im}
        \item {Miete} and {Mitte}
        \item {Ofen} and {offen}
        \item {Ruhm} and {rum}
        \item {Staat} and {Stadt}
        \item {wen} and {wenn}
    \end{itemize}
\end{enumerate}
